...
(function() {
  ...
  st.AI_PROMPT = "Read this page and give me a short summary of the key points:";

  st.AI_COPY_MESSAGE = "Prompt copied — paste it into the chat";

  st.AI_COPY_DELAY = 600;
  ...
  st.copyToClipboard = function(text) {
    var $el, copied, ref;
  ...
  st.getAiPrompt = function(config) {
    var ai_prompt, lines, share_url, title;
    if (config == null) {
      config = {};
    }
    ai_prompt = config.ai_prompt, share_url = config.share_url, title = config.title;
    if (ai_prompt) {
      return ai_prompt;
    }
    lines = [st.AI_PROMPT];
  ...
  st.getShareLabel = function(network, language) {
    ...
    value = '';
    if (st.isAiNetwork(network)) {
      return st.capitalize(st.i18n['ask'][language]);
    }
    if (st.isAiCopyNetwork(network)) {
      return st.capitalize(st.i18n['copy prompt'][language]);
    }
    switch (network) {
      case 'blm':
  ...
  st.isAiNetwork = function(network) {
    return indexOf.call(st.aiNetworks || [], network) >= 0;
  };

  st.isAiCopyNetwork = function(network) {
    return indexOf.call(st.aiCopyNetworks || [], network) >= 0;
  };
  ...
  st.notify = function(network, message) {
    var $btn, $msg, left, rect, top, width;
    $btn = document.querySelector("div[data-network=\"" + network + "\"]");
    if (!$btn) {
      return;
    }
    $msg = document.createElement('div');
    $msg.className = 'st-notify copy-notify';
    $msg.innerText = message;
    $msg.style.cssText = st.FONT_FAMILY + "\n" + (st.BORDER_RADIUS(4)) + "\nbackground: rgba(0, 0, 0, .85);\ncolor: #fff;\nfont-size: 12px;\nleft: 0;\nline-height: 1.4;\nopacity: 0;\npadding: 8px 10px;\npointer-events: none;\nposition: fixed;\ntop: 0;\ntransition: opacity .2s ease-in;\nwhite-space: nowrap;\nz-index: 99999;";
    document.body.appendChild($msg);
    rect = $btn.getBoundingClientRect();
    width = st.getWindowSize().width;
    left = Math.min(Math.max(8, rect.left + (rect.width - $msg.offsetWidth) / 2), width - $msg.offsetWidth - 8);
    top = rect.top - $msg.offsetHeight - 8;
    if (top < 8) {
      top = rect.bottom + 8;
    }
    $msg.style.left = st.px(left);
    $msg.style.top = st.px(top);
    setTimeout((function() {
      return $msg.style.opacity = '1';
    }), 10);
    return setTimeout((function() {
      $msg.style.opacity = '0';
      return setTimeout((function() {
        return st.remove($msg);
      }), 300);
    }), 2500);
  };
  ...
  st.share = function(config) {
    var ai_prompt, channel_is_ssb, channels_displayed, count_url, description, hostname, image, is_android, is_ios, message, network, product, redirects, share_url, short_url, subject, title, url, username, wechat;
    if (config == null) {
      config = {};
    }
    ai_prompt = config.ai_prompt, channel_is_ssb = config.channel_is_ssb, channels_displayed = config.channels_displayed, count_url = config.count_url, subject = config.subject, share_url = config.share_url, short_url = config.short_url, url = config.url, description = config.description, image = config.image, message = config.message, network = config.network, title = config.title, username = config.username;
    ...
    if (network === 'copy') {
      st.copyToClipboard(window.location.href);
      st.notify(network, 'URL Copied!');
    }
    ...
    redirects = {
      ...
      chatgpt: "https://chatgpt.com/?" + st.qs({
        q: ai_prompt,
        hints: 'search'
      }),
      claude: "https://claude.ai/new?" + st.qs({
        q: ai_prompt
      }),
      ...
      mastodon: "https://share.joinmastodon.org?" + st.qs({
        text: title + ' ' + share_url
      }),
      ...
      perplexity: "https://www.perplexity.ai/search/new?" + st.qs({
        q: ai_prompt
      }),
    ...
    if (st.isAiNetwork(network)) {
      return st.openTab(redirects[network]);
    }
    if (st.isAiCopyNetwork(network)) {
      st.copyToClipboard(ai_prompt);
      st.notify(network, st.AI_COPY_MESSAGE);
      setTimeout((function() {
        if (!st.openTab(redirects[network])) {
          return st.notify(network, 'Prompt copied. Allow pop-ups to open the chat.');
        }
      }), st.AI_COPY_DELAY);
      return;
    }
...
(function() {
  ...
  AI_PATHS = {
    ...
    copilot: {
      view_box: '0 0 24 24',
      fill_rule: 'evenodd',
      d: 'M8.3 3.001c-1.245 0-2.45.61-3.13 1.716c-.526.856-1.214 2.064-1.633 3.162c-.947 2.482-1.443 4.561-1.268 6.085c.092.79.375 1.528.969 2.061c.594.534 1.363.74 2.175.74h.055c.38 0 .716.242.837.602l.553 1.661a2.88 2.88 0 0 0 2.734 1.97H15.7c1.245 0 2.45-.608 3.13-1.715c.526-.856 1.214-2.064 1.633-3.162c.947-2.482 1.443-4.561 1.268-6.085c-.092-.79-.375-1.529-.969-2.062s-1.363-.738-2.175-.738h-.055a.88.88 0 0 1-.837-.603l-.553-1.661A2.88 2.88 0 0 0 14.408 3zm5.814 6.235h-1.049c-.805 0-1.515.528-1.746 1.299l-.95 3.17q-.174.571-.483 1.06h1.049c.805 0 1.515-.529 1.746-1.3l.95-3.17q.174-.571.483-1.06m-1.424-3.27A1.35 1.35 0 0 1 13.986 5h.422c.38 0 .716.243.837.603l.543 1.632h-2.723q-.4 0-.78.08zm-1.38 12.07a1.35 1.35 0 0 1-1.296.964h-.422a.88.88 0 0 1-.836-.603l-.544-1.632h2.723q.4 0 .78-.08z'
    },
    gemini: {
      view_box: '0 0 24 24',
      fill_rule: 'nonzero',
  ...
  ai = function(network, fill) {
    var d, fill_rule, ref, view_box;
    if (fill == null) {
      fill = '#fff';
    }
    ref = AI_PATHS[network], d = ref.d, fill_rule = ref.fill_rule, view_box = ref.view_box;
    return "<svg fill=\"" + fill + "\" fill-rule=\"" + fill_rule + "\" preserveAspectRatio=\"xMidYMid meet\" height=\"1em\" width=\"1em\" viewBox=\"" + view_box + "\"><path d=\"" + d + "\"></path></svg>";
  };
  ...
  window.__sharethis__.ICONS = {
    ...
    googlescholar: img('googlescholar.svg'),
    ...
    researchgate: img('researchgate.svg'),
    ...
    truthsocial: img('truthsocial.svg'),
  ...
  window.__sharethis__.ICONS_WHITE = {
    ...
    chatgpt: ai('chatgpt', window.__sharethis__.COLORS['chatgpt']),
    claude: ai('claude', window.__sharethis__.COLORS['claude']),
    copilot: ai('copilot', window.__sharethis__.COLORS['copilot']),
    ...
    gemini: ai('gemini', window.__sharethis__.COLORS['gemini']),
    getpocket: img('pocket-white.svg'),
    gettr: img('gettr-white.svg'),
    github: img('github-white.svg'),
    gitlab: img('gitlab-white.svg'),
    gmail: img('gmail-white.svg'),
    googlebookmarks: img('googlebookmarks-white.svg'),
    googlescholar: img('googlescholar-white.svg'),
    googlemaps: img('googlemaps-white.svg'),
    grok: ai('grok', window.__sharethis__.COLORS['grok']),
    ...
    mastodon: img('mastodon-white.svg'),
    ...
    parler: img('parler-white.svg'),
    patreon: img('patreon-white.svg'),
    perplexity: ai('perplexity', window.__sharethis__.COLORS['perplexity']),
    ...
    researchgate: img('researchgate-white.svg'),
    ...
    truthsocial: img('truthsocial-white.svg'),
...
(function() {
  ...
  st.networks = ['airbnb', 'amazon', 'blogger', 'bluesky', 'buffer', 'chatgpt', 'claude', 'copilot', 'copy', 'diaspora', 'diigo', 'digg', 'discord', 'douban', 'email', 'evernote', 'etsy', 'facebook', 'fark', 'flipboard', 'foursquare', 'gab', 'gemini', 'getpocket', 'github', 'gmail', 'googlebookmarks', 'grok', 'hackernews', 'houzz', 'instapaper', 'kakao', 'kindleit', 'line', 'linkedin', 'livejournal', 'mastodon', 'mailru', 'meneame', 'messenger', 'microsoftteams', 'naver', 'nextdoor', 'odnoklassniki', 'outlook', 'perplexity', 'pinboard', 'pinterest', 'plurk', 'print', 'qzone', 'reddit', 'refind', 'renren', 'sharethis', 'sms', 'snapchat', 'telegram', 'tencentqq', 'threads', 'threema', 'tiktok', 'trello', 'tripadvisor', 'tumblr', 'twitter', 'vk', 'viber', 'vimeo', 'wechat', 'weibo', 'whatsapp', 'wordpress', 'xing', 'yahoomail', 'yummly', 'zillow'];

  st.aiNetworks = ['chatgpt', 'claude', 'grok', 'perplexity'];

  st.aiCopyNetworks = ['copilot', 'gemini'];

  st.shareNetworks = ['blogger', 'bluesky', 'buffer', 'chatgpt', 'claude', 'copilot', 'copy', 'diaspora', 'digg', 'diigo', 'douban', 'email', 'evernote', 'facebook', 'fark', 'flipboard', 'gab', 'gemini', 'getpocket', 'gmail', 'googlebookmarks', 'grok', 'hackernews', 'houzz', 'instapaper', 'kakao', 'kindleit', 'line', 'linkedin', 'livejournal', 'mastodon', 'mailru', 'meneame', 'messenger', 'microsoftteams', 'naver', 'nextdoor', 'odnoklassniki', 'outlook', 'perplexity', 'pinboard', 'pinterest', 'plurk', 'print', 'qzone', 'reddit', 'refind', 'renren', 'sharethis', 'sms', 'telegram', 'tencentqq', 'threema', 'trello', 'tumblr', 'twitter', 'vk', 'viber', 'wechat', 'weibo', 'whatsapp', 'wordpress', 'xing', 'yahoomail', 'yummly'];

  st.followNetworks = ['airbnb', 'amazon', 'bandcamp', 'behance', 'bitbucket', 'blogger', 'bluesky', 'deviantart', 'digg', 'discord', 'dribbble', 'etsy', 'facebook', 'flickr', 'flipboard', 'foursquare', 'gettr', 'github', 'gitlab', 'goodreads', 'googlemaps', 'googlescholar', 'houzz', 'instagram', 'linkedin', 'mastodon', 'medium', 'meetup', 'messenger', 'mix', 'odnoklassniki', 'parler', 'patreon', 'pinterest', 'quora', 'reddit', 'researchgate', 'shares', 'slideshare', 'snapchat', 'soundcloud', 'spotify', 'stackoverflow', 'strava', 'telegram', 'threads', 'tiktok', 'trello', 'tripadvisor', 'truthsocial', 'tumblr', 'twitch', 'twitter', 'vimeo', 'vk', 'wechat', 'weibo', 'whatsapp', 'xing', 'yelp', 'youtube', 'zillow', 'zomato'];
...
(function() {
  ...
  st.loader['image-share-buttons'] = function(config) {
    var $el, $image, $images, ai_prompt, alignment, container, enter, exit, i, id, inline_buttons, len, networks, omit_class, onLoad, over_buttons, over_image, padding, radius, ref, size, spacing, src, timeout;
    if (config == null) {
      config = {};
    }
    if (!config.enabled) {
    ...
    ai_prompt = config.ai_prompt, alignment = config.alignment, container = config.container, omit_class = config.omit_class, onLoad = config.onLoad, padding = config.padding, networks = config.networks, radius = config.radius, size = config.size, spacing = config.spacing;
    if (networks == null) {
...
(function() {
  ...
  DOMAINS = {
    ...
    googlemaps: '',
    goodreads: '',
    googlescholar: 'https://scholar.google.com/citation?user=',
...
(function() {
  ...
  load = function($el, config) {
    ...
    fn = function($button) {
      return st.addEventListener($button, 'click', function() {
        return st.share({
          ai_prompt: ai_prompt || $el.getAttribute('data-ai-prompt'),
...
(function() {
  ...
  load = function($el, config) {
    ...
    fn = function($button) {
      return st.addEventListener($button, 'click', function() {
        return st.share({
          ai_prompt: ai_prompt || ($el != null ? $el.getAttribute('data-ai-prompt') : void 0),
          channel_is_ssb: config.is_ssb,
          channels_displayed: JSON.stringify(config.networks),
...
(function() {
  ...
  st.loader['video-share-buttons'] = function(config) {
    var ai_prompt, alignment, container, networks, omit_class, onLoad, padding, radius, size, spacing;
    if (config == null) {
      config = {};
    }
    if (!config.enabled) {
...
(function(){window.__sharethis__.md5="419e9c89a4c8d0f06d09b285affb5399";})();